/**
 * 前端架构文件，集成了xxx.js和jquery.js ,实现了模块管理和DOM操作
 @author fenglinzeng@gmail.com
 */

/**
 * xxx/1.0.1/xxx.js
 * xxx.js 1.0.1 | xxx.org/LICENSE.md
 */

let xxxjs, xxx, define;
(function(global, undefined) {
    let req, xxx, xxxjs;
    // Avoid conflicting when `xxx.js` is loaded multiple times
    ...
    if (typeof global.xxxjs !== 'undefined') {
        if (isFunction(xxxjs)) {
            // Do not overwrite an existing xxxjs instance.
            return;
        }
        cfg = global.xxxjs;
        global.xxxjs = undefined;
    }

    // Allow for a xxx config object
    if (typeof global.xxx !== 'undefined' && !isFunction(global.xxx)) {
        // assume it is a config object.
        cfg = global.xxx;
        global.xxx = undefined;
    }

    req = xxx = {
        // The current version of xxx.js being used
        version: '1.0.1'
    };

    const data = req.settings = {};
    ...
    // for all events
    xxx.off = function(name, callback) {
        // Remove *all* events
        if (!(name || callback)) {
            events = data.events = {};
            return xxx;
    ...
    const emit = xxx.emit = function(name, data) {
     ...
     * @param refUri
     * @returns {*}
     * @update 20161026 fenglinzeng 相对路径都基于base地址,优化了Add default protocol的判断
     */
    ...
    // Recommend to add `xxxnode` id for the `xxx.js` script element
    const loaderScript = doc.getElementById('xxxnode') ||
        scripts[scripts.length - 1];

    // When `xxx.js` is inline, set loaderDir to current working directory
    ...
    Module.prototype.fetch = function(requestCache) {
        ...
        function sendRequest() {
            xxx.request(emitData.requestUri, emitData.onRequest, emitData.charset);
        }
    ...
    Module.resolve = function(id, refUri) {
        ...
        return emitData.uri || xxx.resolve(emitData.id, refUri);
    };
    ...
    Module.define = function(id, deps, factory) {
        ...
        // Emit `define` event, used in nocache plugin, xxx node version etc
        emit('define', meta);
    ...
    xxx.config = function(configData) {
        eachProp(configData, (value, key) => {
        ...
        if (hasProp(data, 'vars')) {
            global.xxx.data = data.vars;
        }
        emit('config', configData);
        return xxx;
    };
    global.xxxjs = global.xxx = req.use;
    mixin(global.xxx, req);
}(typeof window !== 'undefined' ? window : this));
/**
 * combo.js for xxx.js
 * The xxx.js plugin for concatenating HTTP requests
 */

(function(global) {
    'use strict';

    const xxxjs = global.xxxjs;
    const Module = xxxjs.Module;
    const FETCHING = Module.STATUS.FETCHING;

    const data = xxxjs.settings;
    const comboHash = data.comboHash = {};

    let comboSyntax = ['??', ','];
    let comboMaxLength = 2000;
    let comboExcludes;
    const comboSuffix = function() {
        return '?_' + data.ver;
    };

    xxxjs.on('load', setComboHash);
    xxxjs.on('fetch', setRequestUri);
    ...
    function uris2meta(uris) {
        ...
        for (let i = 0, len = uris.length; i < len; i++) {
            const parts = uris[i].replace('://', '__').split('/');
            let m = meta;

            for (let j = 0, l = parts.length; j < l; j++) {
                const part = parts[j];
    ...
    function meta2paths(meta) {
        const paths = [];
        const __KEYS = meta.__KEYS;

        for (let i = 0, len = __KEYS.length; i < len; i++) {
            const part = __KEYS[i];
            let root = part;
            let m = meta[part];
            let KEYS = m.__KEYS;
    ...
    function paths2hash(paths) {
        for (let i = 0, len = paths.length; i < len; i++) {
            const path = paths[i];
            const root = path[0] + '/';
            const group = files2group(path[1]);
    ...
    function setHash(root, files) {
        ...
        let comboPath = root + comboSyntax[0] + copy.join(comboSyntax[1]);
        if (comboSuffix) {
            comboPath += comboSuffix();
        }
        const exceedMax = comboPath.length > comboMaxLength;
    ...
    function isComboUri(uri) {
        const comboSyntax = data.comboSyntax || ['??', ','];
        const s1 = comboSyntax[0];
        const s2 = comboSyntax[1];
    ...
    // For test
    if (data.test) {
        const test = xxxjs.test || (xxxjs.test = {});
...
(function(global) {
    ...
    const vars = global.pageConfig || (global.pageConfig = {});
    // 设置当前协议
    ...
    if (!vars.base) {
        ...
        const hrefArrLen = hrefArr.length;
        let publicUrl = '';
        if (hrefArrLen >= 3) {
            const last = hrefArr[hrefArrLen - 1];
            if (/.+\.\w+/.test(last) || !last) {
                hrefArr.pop();
            }
    ...
    const config = {
        ...
        alias: {
            jquery: 'jquery',
            jqweui: 'lib/jquery-weui.js',
            jweixin: '//res.wx.qq.com/open/js/jweixin-1.2.0.js',
            swiper: 'lib/swiper.min.js',
            socket: 'lib/web_socket.js',
            swfobject: '//cdn.bootcss.com/swfobject/2.2/swfobject.min.js',
